...
public class OceanCanvas extends Canvas {
	 ...
	 *
	 * @param fishWindow
	 *            The Window commanding the Ocean.
	 * @param h
	 *            Height in number of fish.
	 * @param w
	 *            Width in number of fish.
	 */
	public OceanCanvas(FishWindow fishWindow, int h, int w) {
		...
		/*
		 * Surface waves should alwas be last in the background list and therefore it is always drawn above all other
		 * background objects.
		 */
		...
		/*
		 * Force creating of the bufferImage the first time paint() is called.
		 */
		bufferImage = null;
	 ...
	 *
	 * @param newMatrix
	 *            Matrix containing the fish or <tt>null</tt> for water squares.
	 */
	...
	/**
	 * Paint the ocean. Since we use double buffering we only have to redisplay the buffer. Create a buffer when called
	 * for the first time.
	 *
	 * @param g
	...
	/**
	 * Repaint everything. Does not clear the canvas before painting as it gets completely overwritten.
	 *
	 * @param g
	 *            Graphics object to paint into.
	 */
	...
	/**
	 * Create a ship if there isn't one visible at the moment and if Math.random() indicates it.
	 */
	...
	protected void createWhaleAtRandom() {
		...
		backgroundObjects.add(new Whale(this, ImageData.WHALE, zero, amplitude, wavelength,
	...
	/**
	 * Create a submarine if there isn't once visible at the moment and if Math.random() indicates it.
	 */
	protected void createSubmarineAtRandom() {
		...
		backgroundObjects.add(new Submarine(this, ImageData.SUBMARINE, zero + waterTop, amplitude, wavelength, random
				.nextDouble() * 2 + .5));
	}
	...
	protected void paintBackground(Graphics g) {
		...
		g.fillRect(0, waterTop, widthInPixels, waterHeight + bottomHeight - groundHeight);
	 ...
	 *                Thrown if coordinate is invalid.
	 */
