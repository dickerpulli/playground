...
public class Surface extends AnimationObject
{
   ...
   * 
   * @param ocean The ocean needing a surface.
   */
  ...
  /**
   * Animate the surface.
   * The surface never requests to be deleted.
   *
   * @return Always false.
   */
  public void tick()
  {
    ...
    waveLength = waveLengthAmplitude * Math.cos(waveLengthPhase) 
      + waveLengthZero;    
  }
  ...
  {     
    ...
    for(int x = 0; x < ocean.getWidthInPixels(); x += stepSize)
      {
	y = (int)Math.round(amplitude * Math.sin(phase + x / waveLength)) 
	  + zeroline;
	p.addPoint(x, y);
      }
