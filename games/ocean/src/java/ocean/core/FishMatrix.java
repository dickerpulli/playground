...
/** A matrix of fish of constant size.<p>
 * 
 * There are access method <code>getFish</code> and <code>setFish</code> to 
 * request or set a fish on givern coordinates.
 * 
 * It is possible to add a <em>direction</em> parameter <code>getFish</code> or 
 * <code>setFish</code>, to get/set the position one step in that direction.
 * 
...
public abstract class FishMatrix
{
	...
	{
		return (0 <= coord.y && coord.y < getHeight()) 
			&& (0 <= coord.x && coord.x < getWidth());
	}
	
	/**
	 * tells whether there is a cell in the direction dir from 
	 * the cell (x,y) in the matrix
	...
	{
		if (!validCoordinate(from)) throw new IllegalArgumentException("invalid coordinates: "+ from);
		
		if (getCoordinate(from, dir) == null) return false;
		return true;
	}
	...
	{
		if (!(validCoordinate( coord ) )) throw new IllegalArgumentException("invalid coordinates : " + coord);
		return ( matrix[ coord.y ][ coord.x ]) ;
	}

	/**
	 * Stores a fish on the coordinates <code>(x,y)</code>. 
	 * It's allowed to pass <code>null</code> in order to clear this cell
	 * and delete the fish which was stored.
	 *
	 * @param coord The coordinate
	 * @param fish the fish to be stored
	 * 
	 * @throws IllegalArgumentException if the cell does not exist
	 */
	public void setFish( Coordinate coord, Fish fish ) throws IllegalArgumentException 
	{
		...
		else throw new IllegalArgumentException("invalid coordinates : " + coord);
	}

    /**
     * Returns the Coordinate of the cell reached when starting in 
     * cell <code>from</code> and moving one step in direction <code>dir</code>. 
     *
     * @param from the Coordinate where we start 
    ...
    public Coordinate getCoordinate ( Coordinate from, Direction dir ) {
    	...
    	if (validCoordinate(to)) return to;
    	
		return null;
    }

	/**
	 * Returns the fish in the cell reached when starting in cell <code>(x,y)</code> 
	 * and moving one step in direction <code>direction</code>. 
	 *
	 * @param from The start position.
	 ...
	 * @param fish The fish to set at the new location.
	 * @param from The coordinate where the fish should be removed.
	 * @param to The coordinate where the fish should be moved to.
	 * 
	...
	/**
	 * Returns the coordinate of a Fish object in the matrix.
	 * Note that this does not use equals(), but really looks for the same object.
	 * 
	 * @param fish The fish to find.
	...
	public Coordinate whereIsMyFish(Fish fish) {
		...
		for (int x = 0; x < getWidth(); x++)
			for (int y = 0; y < getHeight(); y++) {
				...
				if (getFish(where) == fish) return where;
			}
	 ...
	 * @param manyFish The List containing all the Fish to be inserted.
	 */	
	...
	/**
	 * Passes the doSomething-command to the fish at the specified position.
	 * If there is no fish, it reports that.
	 * 
	 * @param where The coordinate that was prompted to do something.
