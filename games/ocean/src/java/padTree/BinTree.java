...
public abstract class BinTree<T> {
	...
	public int getSize(BinTreeNode Node) { // SizeOfTheSubtree
		return (Node == null) ? 0 : 1 + getSize(Node.getLeftChild())
	...
	public String toString(BinTreeNode Node) {
		return (Node == null) ? "*" : "(" + toString(Node.getLeftChild()) + ")"
				+ Node.toString() + "(" + toString(Node.getRightChild()) + ")";
	...
	public void increment() {
		...
		} while (Origin != OriginEnum.LEFT
				&& !(Origin == OriginEnum.ABOVE && _curr.getLeftChild() == null));
	}// increment()
